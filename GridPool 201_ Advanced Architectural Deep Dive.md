...
* **The "Protection Fee" Trade-off:**  The protocol allows for a tunable "Protection Fee" split. By increasing the reward allocated to Slot 0 relative to the 299 other slots, miners can adjust the dial between "pure solo mining" and "shared lottery mining." A higher Protection Fee makes block withholding exponentially more expensive for an attacker, as they forfeit a larger guaranteed reward by failing to broadcast.  
...
#### 7\. Consensus Mechanisms: Dynamic Team Convergence

A "Team" is defined as a loose convergence of miners working on templates derived from the same current payout list, or **Winners List (WL)** . To handle the realities of network latency and race conditions, GridPool applies a variant of the "Heaviest Chain" rule: the  **Strongest Team Rule**.  
**Conflict Resolution:**  When network latency causes divergent Winners Lists (e.g., if a new high-difficulty share is discovered simultaneously with a Bitcoin block), there could arise a “pool split”, analogous to a chain split in P2Pool like methods.  In this case, some miners will be using a Winners List which includes the last minute share, and some will not, depending on whether they observed the new Bitcoin block first or the last minute share first. Both competing lists are shared widely across the network.  
In this event, nodes calculate the total difficulty of each list by summing the proofs-of-work of all listed shares. The Winners List representing the highest cumulative difficulty is selected as the primary list. Nodes naturally converge on this "Strongest Team," resolving splits and joins during the transition between Bitcoin blocks.  This also serves as a censorship resistance mechanism.  Miners that choose to censor valid shares will fork themselves off to a lower total difficulty set of lists.  Game theoretically, miners are incentivized to join the group with the highest total hashpower, as this maximizes their chance of winning a real payout. 

#### 8\. Algorithmic Scaling: The Dynamic WL Threshold Difficulty

GridPool maintains network performance through a dynamic  **WL Threshold Difficulty** , which balances participation accessibility against bandwidth constraints.**The Formula:**  The threshold is defined as:WL Threshold \= 0.5 \* (Difficulty of the lowest proof in the previous round's Winners List).  This is primarily designed to prevent a DDoS like “share storm” of low difficulty shares immediately after the start of a new round.    

#### 9\. Sybil Resistance

GridPool is fundamentally Sybil-resistant because attribution is tied to verified Proof-of-Work. Identities do not vote; only the realized difficulty of submitted shares determines ranking within the Winners List.  
